    ...
    */
    ...
    pageScript: function () { //scripts to be executed on each view change
        ...
        $(document).ready(function () {
            ...
            var isInSelectedRange = function(date){
              if(self.dateFromSelected===null || self.dateToSelected===null){
                return false;
              }
              var time = date.getTime();
              return self.dateFromSelected<=time && self.dateToSelected>=time;
            }

            var refreshDatepickers = function(){
                $("#date-from").datepicker("refresh");
                $("#date-to").datepicker("refresh");
            }

            var beforeShowDayFn = function(date){
                var inRange = isInSelectedRange(date);
                return [inRange, inRange ? "in-range-highlight" : ""]
            }
